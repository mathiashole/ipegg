...
if (!is.null(words_to_remove) && length(words_to_remove) > 0) {
  ...
  message("No words specified for removal. Skipping domain filtering.")
}
...
if (!is.null(tree_file)) {
  ...
  unique_ids <- as.character(unique(df_filtered$V1))
  final_order <- unique_ids[order(xtfrm(unique_ids))]
} else {
...
dynamic_height <- 1.0 + (n_seqs * 0.18)

dynamic_height <- min(dynamic_height, 85)
